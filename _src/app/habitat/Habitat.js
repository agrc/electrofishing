  ...
  return declare([_WidgetBase, _TemplatedMixin, _ClearValuesMixin, _InProgressCacheMixin, _GridMixin], {
    ...
    loadComboboxes() {
      var lst = new DeferredList([
        ...
        Domains.populateSelectWithDomainValues(
          this.overstorySelect,
          config.urls.habitatFeatureService,
          config.fieldNames.habitat.DOVR
        ),
        Domains.populateSelectWithDomainValues(
          this.understorySelect,
          config.urls.habitatFeatureService,
    ...
    onRemoveTransect: function (event) {
      ...
      this.store
        .filter(this.getTransectFilter(event.tabNum))
    ...
    getTransectData: function () {
      ...
      return Object.keys(this.transects).map(
        function (key) {
          var obj = {};
    ...
    _validateWidth(wettedWidth) {
      ...
      let message = isValid ? null : 'The distance from the starting bank needs to be less than the wetted width.';
