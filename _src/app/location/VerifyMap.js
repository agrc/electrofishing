  ...
  return declare([_WidgetBase, _TemplatedMixin, _SubscriptionsMixin], {
    ...
    initMap: function () {
      ...
      this.addSubscription(
        topic.subscribe(config.topics.mouseWheelZooming_onChange, function (_, enable) {
          if (enable) {
      ...
      L.tileLayer(config.urls.googleImagery, { quadWord: config.quadWord }).addTo(this.map);
      L.tileLayer(config.urls.overlay, { quadWord: config.quadWord }).addTo(this.map);
          ...
          onEachFeature: function (geojson, layer) {
            ...
            layer
              .on('mouseover', function () {
                popup
                  .setContent(dojoString.substitute(stationPopupTemplate, replaceNulls(geojson.properties)))
      ...
      this.streamsLyr = L.esri
        .featureLayer({
          url: config.urls.streamsFeatureService,
        })
        .addTo(this.map);
      this.lakesLyr = L.esri
        .featureLayer({
