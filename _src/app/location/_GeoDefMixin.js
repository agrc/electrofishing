define(['dojo/Deferred', 'dojo/promise/all', 'dojo/request/xhr', 'dojo/_base/array', 'dojo/_base/declare'], function (
  ...
  return declare(null, {
    ...
    checkJobStatus: function (jobId, def, intId) {
      ...
      xhr(this.gpServiceUrl + '/jobs/' + jobId + '?', this.defaultXHRParams).then(
        function (jobStatus) {
    ...
    getJobResults: function (jobId, def) {
      ...
      function getResult(paramName, successCallback) {
        ...
        return xhr(that.gpServiceUrl + '/jobs/' + jobId + '/results/' + paramName + '?', that.defaultXHRParams).then(
          function (results) {
            if (results) {
      ...
      promises.push(
        getResult('segmentWGS', function (value) {
      ...
      promises.push(
        getResult('segmentUTM', function (value) {
          if (value.features.length > 0) {
      ...
      promises.push(
        getResult('error_message', function (value) {
          error_message = value;
        })
      );
