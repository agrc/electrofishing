  ...
  return declare(mixins, {
    ...
    postCreate: function () {
      ...
      Domains.populateSelectWithDomainValues(
        this.streamTypeSelect,
        config.urls.stationsFeatureService,
        config.fieldNames.stations.STREAM_TYPE
      ).then(initCombobox);
    ...
    wireEvents: function () {
      ...
      this.addSubscription(topic.subscribe(config.topics.pointDef_onBtnClick, this.onPointDefSelected.bind(this)));
    },
    ...
    onWaterBodyClick(event) {
      ...
      this.streamLakeInput.value = event.layer.feature.properties[config.fieldNames.reference.Permanent_Identifier];
    },
  });
});
