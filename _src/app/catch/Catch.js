define([
  ...
  return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, _GridMixin], {
    ...
    wireBatchFormEvents: function () {
      ...
      this.own(on(this.batchWeightTxt, 'keyup, change', lang.hitch(this, 'validateBatchForm')));
    },
    ...
    moreInfo: function (evt) {
      ...
      if (row && row.data[this.COUNT] === 1) {
        this.moreInfoDialog.show(row.data[config.fieldNames.fish.FISH_ID], evt.target.dataset.tab);
      }
    },
    clear: function () {
      // summary:
